...
export default function Home({ onNavigate }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // call resume endpoint (unchanged)
      const pdfResp = await axios.post(
        `${url}/resume`,
        { ...resp.data, matcher: sm.data },
        { headers: { "Content-Type": "application/json" }, responseType: "blob" }
  ...
  return (
    <div className="space-y-10">
      <LoaderOverlay show={loading} message="Generating insights — this may take 1–2 minutes" />
      <div>
        <h1 className="text-4xl font-bold heading mb-2">Your Career, Simplified</h1>
        <p className="muted text-sm">Upload your resume and job description to generate an ATS-optimized resume, identify skill gaps, and find matching jobs.</p>
      </div>
      ...
      {error && (
        <div className="glass p-4 border border-red-600/20 text-red-200 rounded-md">
          {error}
        </div>
      )}

      <div className="flex items-center gap-4">
        <button onClick={handleSubmit} disabled={loading} className="btn px-6 py-3 font-semibold">
          {loading ? "Processing..." : "Generate Insights"}
        </button>
        ...
        }} className="btn ghost">Clear saved</button>

        <div className="ml-auto muted small">{loading ? "Working..." : processOutput ? "Ready — view results below" : "Waiting for input"}</div>
      </div>

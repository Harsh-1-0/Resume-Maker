...
const API_URL = import.meta?.env?.VITE_API_URL || process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000/process_pair/';

function Badge({ children }) {
  return <span className="px-2 py-1 rounded bg-white/10 text-sm">{children}</span>;
}

export default function ProcessPairPage() {
  // resume inputs
  ...
  useEffect(() => {
    // Load cached process_pair if present (preview)
    const cached = localStorage.getItem('process_pair_output');
    if (cached) {
      try { setResponseJson(JSON.parse(cached)); } catch {}
    }
  }, []);
  ...
  function clearAll() {
    ...
    if (fileInputJdRef.current) fileInputJdRef.current.value = '';
  ...
  async function submitPair(e) {
    ...
    try {
      const res = await fetch(API_URL, {
        method: 'POST',
        body: form,
        // intentionally NO Content-Type header so browser sets multipart/form-data boundary
        headers: {
      ...
      let data;
      try { data = await res.json(); } catch (err) { data = { raw: 'invalid json response' }; }
      ...
      if (!res.ok) {
        ...
        setResponseJson(data);
        try { localStorage.setItem('process_pair_output', JSON.stringify(data)); } catch {}
      }
    } catch (err) {
      setError(String(err));
    } finally {
  ...
  return (
    <div className="min-h-screen flex items-start justify-center p-6 bg-slate-900 text-white">
      <div className="max-w-5xl w-full bg-gradient-to-b from-slate-800/60 to-slate-900/60 border border-white/5 rounded-2xl p-6 shadow-2xl">
        <header className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-semibold">Resume Maker — Test UI</h1>
            <p className="text-sm text-slate-300">Upload Resume + Job Description and get structured JSON outputs.</p>
          </div>
          <div className="text-right"><Badge>FastAPI</Badge></div>
        </header>
        ...
        <form onSubmit={submitPair} className="grid gap-6 md:grid-cols-2">
          ...
          <div className="md:col-span-2 flex gap-4 items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <button type="button" onClick={clearAll} className="px-3 py-2 bg-white/5 rounded text-white">Clear</button>
            </div>
          ...
          <div className="md:col-span-2 grid grid-cols-2 gap-4">
            <div className="bg-white/5 p-3 rounded min-h-[180px]">
              ...
              {responseJson && <pre className="text-slate-200 whitespace-pre-wrap">{pretty(responseJson)}</pre>}
